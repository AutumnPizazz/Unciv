...
- 更新日志：任何改动合入时同步在中英 `docs/{,zh/}UncivCN/Changelog.md` 顶部的「未发布」小节各加一行，发版时整合为新版本条目（详见 Coding-standards 第 5 节）；上游日志英文页 `docs/Community/Upstream-changelog.md` 构建时自动嵌入根目录 `changelog.md`（永远最新），中文页 `docs/zh/Community/Upstream-changelog.md` 只维护「最近版本翻译」（链接英文页看完整原文）
