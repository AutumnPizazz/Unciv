...
enum class UniqueType(
    ...
    MaxHealth("[relativeAmount] Max HP", UniqueTarget.Unit, UniqueTarget.Global,
        docDescription = "Negative amounts are allowed; a resulting Max HP of 0 or less is reset to 1. Only plain or \"for [mapUnitFilter] units\"-conditional uniques count: other conditionals are ignored so that Max HP stays constant.",
        docDescriptionZh = "允许负值倒扣；结果不大于 0 时重置为 1。只有无条件或带「for [mapUnitFilter] units」修饰的 unique 才生效：其余条件会被忽略以保证最大生命恒定。"),
    ...
    UnitMaintenanceDiscount("[relativeAmount]% maintenance costs", UniqueTarget.Unit, UniqueTarget.Global,
        docDescription = MULTIPLICATIVE_BONUS_EXPLANATION, docDescriptionZh = MULTIPLICATIVE_BONUS_EXPLANATION_ZH),
    UnitMaintenanceDiscountFlat("Reduces unit maintenance by [amount] Gold", UniqueTarget.Global,
        docDescription = "Flat per-unit gold reduction, applied after percentage discounts. Only takes effect in the Civilization 6 style maintenance system (see 'Uses the Civilization 6 style unit maintenance system'). Combine with unit filters like <for [Military] units> to discount only certain units.",
        docDescriptionZh = "每单位固定金币减免，在百分比折扣之后应用。仅在文明6式维护费体系（见「Uses the Civilization 6 style unit maintenance system」）中生效。可与 <for [Military] units> 等单位过滤器组合，只减免特定单位。"),
    UnitMaintenanceCiv6Style("Uses the Civilization 6 style unit maintenance system", UniqueTarget.ModOptions,
        flags = UniqueFlag.setOfNoConditionals,
        docDescription = "Opt-in for rulesets (put it in ModOptions.json uniques). Enables per-unit fixed maintenance costs from the Units.json 'maintenanceCost' field, the configurable 'unitMaintenanceBaseCost' constant, flat Gold reductions (see 'Reduces unit maintenance by [amount] Gold'), and removes the game-progress maintenance inflation and the 3 default free units.",
        docDescriptionZh = "规则集可选用（放在 ModOptions.json 的 uniques 中）。启用 Units.json 的 maintenanceCost 字段（按单位固定维护费）、可配置的 unitMaintenanceBaseCost 常量、固定金币减免（见「Reduces unit maintenance by [amount] Gold」），并移除随游戏进度增长的维护费膨胀与默认 3 个免费单位。"),
    ...
    TriggerUponTradeMade("upon completing a trade with [civFilter] Civilizations", UniqueTarget.TriggerCondition,
        docDescription = "Triggers when this civ completes a trade (accepted by either side) with a matching civilization. Note: this fires on every accepted trade, including AI auto-accepted ones and open-borders deals made during conquest.",
        docDescriptionZh = "当本文明与匹配的文明完成一笔交易（任意一方接受）时触发。注意：所有被接受的交易都会触发，包括 AI 自动接受与征服时签订的开放边界协议。"),
    ...
    TriggerUponCapturingUnit("upon capturing a [mapUnitFilter] unit", UniqueTarget.UnitTriggerCondition),
