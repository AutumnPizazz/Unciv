        ...
        when (unique.type) {
            ...
            UniqueType.TriggerLuaFunction -> {
                ...
                return {
                    ...
                    var success = false
                    LuaScriptManager.callFunction(luaFunc, ctx, civInfo, functionName, onSuccess = { success = it }, modName = foundMod)
                    success
                }
            }
            ...
            UniqueType.OneTimeGainStatRange -> {
                ...
                // Inclusive range like upstream (includes upper bound, safe for equal bounds),
                // while keeping CN's countable parameter support via resolveAmount
                val randomValue = (minOf(amountLow, amountHigh)..maxOf(amountLow, amountHigh)).random(tileBasedRandom)
