...
class QuestManager : IsPartOfGameInfoSerialization {
    ...
    private fun tryStartNewGlobalQuest() {
        ...
        if (assignableQuests.isNotEmpty()) {
            val quest = assignableQuests.randomWeighted(civ.state.stateBasedRandom("QuestManager.tryStartNewGlobalQuest")) { getQuestWeight(it.name) }
    ...
    private fun tryStartNewIndividualQuests() {
        for ((challengerName, countdown) in individualQuestCountdown) {
            ...
            if (assignableQuests.isNotEmpty()) {
                val quest = assignableQuests.randomWeighted(civ.state.stateBasedRandom("QuestManager.tryStartNewIndividualQuests")) { getQuestWeight(it.name) }
