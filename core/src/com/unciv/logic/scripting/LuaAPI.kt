...
object LuaAPI {
    ...
    fun buildContext(
        ...
        tileStats: LuaValue? = null
    ): LuaValue {
        val ctx = LuaValue.tableOf()
        ctx.registerApi("ctx", "parameter", LuaValue.valueOf(resolvedParam))
        ctx.registerApi("ctx", "value", if (value != null) LuaValue.valueOf(value) else LuaValue.NIL)
        ...
        ctx.registerApi("ctx", "tileStats", tileStats ?: LuaValue.NIL)
    ...
    private fun buildCivTable(civ: Civilization): LuaValue {
        ...
        t.registerApi("civ", "setVariable", luaFunction { args ->
            civ.setVariable(args.arg(1).tojstring(), args.arg(2).safeToInt())
            LuaValue.NIL
        })
        t.registerApi("civ", "addVariable", luaFunction { args ->
            civ.addVariable(args.arg(1).tojstring(), args.arg(2).safeToInt())
            LuaValue.NIL
        })
    ...
    private fun buildUnitTable(unit: MapUnit): LuaValue {
        ...
        // Unit-scope variables (mod-defined counters, see Variables.json)
        t.registerApi("unit", "getVariable", luaFunction { args ->
            LuaValue.valueOf(unit.getVariable(args.arg(1).tojstring()))
        })
        t.registerApi("unit", "setVariable", luaFunction { args ->
            unit.setVariable(args.arg(1).tojstring(), args.arg(2).safeToInt())
            LuaValue.NIL
        })
        t.registerApi("unit", "addVariable", luaFunction { args ->
            unit.addVariable(args.arg(1).tojstring(), args.arg(2).safeToInt())
            LuaValue.NIL
        })
        t.registerApi("unit", "getVariables", luaFunction {
            val variables = LuaValue.tableOf()
            for ((name, amount) in unit.variables) {
                val variable = unit.civ.gameInfo.ruleset.variables[name]
                if (variable == null ||
                    (variable.resolvedScope == VariableScope.Unit && variable.isAvailableTo(unit.civ)))
                    variables.set(name, LuaValue.valueOf(amount))
            }
            variables
        })
    ...
    private fun buildGameTable(civInfo: Civilization): LuaValue {
        ...
        t.registerApi("game", "doesVariableExist", luaFunction { args ->
            LuaValue.valueOf(ruleset.variables.containsKey(args.arg(1).tojstring()))
        })
