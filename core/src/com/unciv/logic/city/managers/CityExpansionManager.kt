...
class CityExpansionManager : IsPartOfGameInfoSerialization {
    ...
    /**
     * Checks if this city is adjacent to another city.
     * Two cities are considered adjacent if they share at least one border tile.
     */
    @Readonly
    fun isAdjacentTo(otherCity: City): Boolean {
        return city.getTiles().any { ourTile ->
            ourTile.neighbors.any { neighbor ->
                neighbor.getCity() == otherCity
            }
        }
    }

    /**
     * Gets all tiles from this city that are adjacent to the other city.
     * A tile is considered adjacent if it shares a border with a tile owned by the other city.
     */
    @Readonly
    fun getTilesAdjacentTo(otherCity: City): Sequence<Tile> {
        return city.getTiles().filter { ourTile ->
            ourTile.neighbors.any { neighbor ->
                neighbor.getCity() == otherCity
            }
        }
    ...
    /**
     * Gets all exchangeable tiles from this city with the other city.
     * A tile is exchangeable if:
     * - It is not a city center
     * - It is not in the first ring (distance 1 from city center)
     * - It is within the exchangeable distance range
     * - It is adjacent to the other city
     */
    @Readonly
    fun getExchangeableTilesWith(otherCity: City): Sequence<Tile> {
        return getTilesAdjacentTo(otherCity).filter { tile ->
            !tile.isCityCenter() &&
            !isFirstRingTile(tile) &&
            isWithinExchangeRange(tile)
        }
    }

    /**
     * Exchanges a tile with another city.
     * This city will give up ownership of [ourTile] and take ownership of [theirTile].
     *
     * @param otherCity The other city to exchange tiles with
     * @param ourTile A tile owned by this city to give to the other city
     * @param theirTile A tile owned by the other city to give to this city
     *
     * @throws IllegalArgumentException if:
     * - The cities are not adjacent
     * - ourTile is not owned by this city
     * - theirTile is not owned by otherCity
     * - Either tile is a city center
     * - Either tile is in the first ring
     * - Either tile is outside the exchange range
     */
    fun exchangeTileWith(otherCity: City, ourTile: Tile, theirTile: Tile) {
        // Validate that cities are adjacent
        if (!isAdjacentTo(otherCity)) {
            throw IllegalArgumentException("$city is not adjacent to $otherCity")
        }

        // Validate ourTile
        if (ourTile.getCity() != city) {
            throw IllegalArgumentException("$ourTile is not owned by $city")
        }
        if (ourTile.isCityCenter()) {
            throw IllegalArgumentException("$ourTile is a city center and cannot be exchanged")
        }
        if (isFirstRingTile(ourTile)) {
            throw IllegalArgumentException("$ourTile is in the first ring and cannot be exchanged")
        }
        if (!isWithinExchangeRange(ourTile)) {
            throw IllegalArgumentException("$ourTile is outside the exchange range for $city")
        }

        // Validate theirTile
        if (theirTile.getCity() != otherCity) {
            throw IllegalArgumentException("$theirTile is not owned by $otherCity")
        }
        if (theirTile.isCityCenter()) {
            throw IllegalArgumentException("$theirTile is a city center and cannot be exchanged")
        }
        if (otherCity.expansion.isFirstRingTile(theirTile)) {
            throw IllegalArgumentException("$theirTile is in the first ring and cannot be exchanged")
        }
        if (!otherCity.expansion.isWithinExchangeRange(theirTile)) {
            throw IllegalArgumentException("$theirTile is outside the exchange range for $otherCity")
        }
