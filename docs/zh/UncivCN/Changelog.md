...
- Android：更新包下载具备断点救援——安装包先下载为 .part 临时文件、完成后才改名（下载中途退出只会留下半截文件，重开弹窗点击即可重新下载，不会被误判为已下载）；已下载完成的包提供「安装」与「重新下载」两个按钮，并自动复制一份到系统下载文件夹（Android 10+，拒绝安装后也可从文件管理器/下载列表找回安装）；弹窗打开时自动清理旧版本残留安装包
- 修复主菜单「下载最新版本」按钮跳转浏览器打开发布页而非下载安装包：现在从 release 资产中列出当前平台的安装包（Windows 端完整列出 MSI 安装包 / 绿色版 zip / 通用 jar 供玩家自选，其余平台列出各自包，排除服务器 jar 与辅助包），经当前下载源打开所选包的直链下载；更新弹窗同时显示实际文件名与真实版本号
