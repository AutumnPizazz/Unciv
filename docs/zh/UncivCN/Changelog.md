...
- 测试：为联机「禁止读档」进入判定新增单元测试（LocalSnapshotDecisionTest，9 用例）——同回合进入从本地快照继续，其余情形（未开启、单机、游戏内同步、快照缺失/过期/回合不匹配）回退服务器状态；判定逻辑提取为纯函数 resolveLocalSnapshotForEntry（行为不变，既有快照测试与序列化测试保持全绿）

- 修复：地图钉（地块备注）编辑与查看弹窗的预览图不再提前渲染未揭示资源——像素地形图层（默认开启）现同样按当前玩家科技可见性过滤资源，与资源图标层一致（如远古时代地图钉预览不再出现石油）

- 旋转对称地图重构完成：对称不再于地图生成完毕后打补丁，而是生成时就参与——陆地/温湿/起伏/湖泊海岸/植被/冰/河流/自然奇观/资源/遗迹/起始位置全部改为规范扇区决策+轨道即时同步（新增 `MapSymmetry` 轨道权威，含环形地图感知的覆盖空间轨道运算）；修复旧实现三类问题：环形图上 3/6 折对称静默失效（残留地形/温度不对称）、镜像大陆 ID 不一致影响按大陆判定规则、资源/奇观复制时瞬态过期；删除全部旧事后补丁函数与编辑器「Apply rotational symmetry」步骤；生成结束后可开 DEBUG 校验对称性；2/3/6 折 × 环形/非环形全部验证零不对称，生成性能无退化
