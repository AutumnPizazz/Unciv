...
- 旋转对称地图重构（第二步）：陆地/温湿/起伏/湖泊海岸/植被/稀有特征/冰全部改为规范扇区决策+轨道即时同步；河流生成后立即同步河道；删除原 Phase 1/1b 事后补丁，Phase 2/3 换成完整轨道映射的 enforceSymmetry（不再需要跑两遍）；修复 stampInto 因地形瞬态过期导致复制错误、以及旧 applySymmetry 在环形图上静默漏配的问题——2/3/6 折 × 环形/非环形全部验证零不对称
