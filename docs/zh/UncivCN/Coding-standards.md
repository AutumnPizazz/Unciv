...
- **模组条目**：模组/Lua 功能面向模组作者，允许记录，但只写一句「新增了
  ...
  指向细节；字段名/函数名/机制细节一律留给文档，不写进更新日志。
...
- 条目风格：一行一条、短句要点；同一版本内按类别用 emoji 分组小节
  ...
  只写用户可见变化，不写实现细节，**每条 ≤30 字（链接不计），重要改动拆多条**。
