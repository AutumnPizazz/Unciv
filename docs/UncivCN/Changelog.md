...
- Tests: added unit coverage for the "Forbid reload" multiplayer entry decision (LocalSnapshotDecisionTest, 9 cases) - same-turn entry resumes from the local snapshot, everything else (option off, single-player, in-game sync, missing/stale/mismatched snapshot) falls back to the server state; the decision was extracted into a pure `resolveLocalSnapshotForEntry` (behavior unchanged, existing LocalSnapshotTest + serialization tests stay green)
