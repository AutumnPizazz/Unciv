...
- **30-word limit**: each entry is at most 30 words (English counts words, the Chinese copy counts characters; links and punctuation are excluded). Big changes must be split into multiple short entries (one line each, each within the limit) - never one giant line.
- **Player-facing, not implementation notes**: entries describe what players see or experience - features, fixes, behavior changes - never implementation details: no class/function names, no internal mechanisms, no refactor or test specifics. Pure tests and behavior-neutral refactors do not need an entry (at most a one-line "hardened by tests" note when meaningful). Before committing an entry, ask: what does the player experience from this change? If nothing, skip it; if something, phrase it in player terms.
- **Bottom-line changes and optimizations stay brief**: architecture rework, performance work, refactors and upstream merges that players cannot directly perceive collapse into one short entry (e.g. "merged upstream fixes", "performance optimizations") without detail; purely internal changes may be skipped entirely.
- **Modding entries**: mod / Lua features target mod authors, so they may appear - but keep them to one short line ("new uniques for X" / "Lua can now do Y") and link to the docs page with the details (`[uniques](/Modders/uniques)`, `[Lua Modding](/Modders/Lua-Modding)`, …). Never list field/method names or mechanisms in the changelog itself. The 30-word limit applies here too (links excluded); split feature lists into several entries.
...
- CoeHarMod is a git submodule of Unciv (`android/assets/mods/CoeHarMod`, its own repo AutumnPizazz/CoeHarMod); **the actual working branch is `workspace`** (`main` is a release branch generated by a GitHub Action that syncs core files on tags); the mod version lives in `modVersion` of `jsons/ModOptions.json`, the compatible game version in `recommendedGameVersion`
- **Every mod change (feature / bugfix / CI / docs) must also update the docs-site mod section changelog** `docs/Community/Mods/CoeHarMod/changelog/index.md` and its Chinese copy `docs/zh/Community/Mods/CoeHarMod/更新日志/index.md` (EN + ZH): add one line to the **Unreleased** section at the top of each (EN in English, ZH in Chinese)
- Style: one line per entry, short bullet points; when a version has several themes, group entries into emoji category subsections (e.g. `### ⚖️ Maintenance system rework`, `### 🔧 Dev tooling`; the ZH copy uses Chinese section titles like `### ⚖️ 快乐系统优化`, `### 🔧 开发工具`), follow the existing log. Entries are player-facing too - describe what mod users see, not implementation details - and follow the same 30-word / 30-character limit (links and punctuation excluded); split big changes into multiple entries.
- Consolidate on release: update the `::: tip Latest version` block (`**vX.Y.Z** - adapted for Unciv A.B.C.D`; the ZH copy uses `::: tip 最新版本` and `- 适配 A.B.C.D`), move the whole Unreleased section into the new `## vX.Y.Z - for A.B.C.D` entry (ZH copy: `## vX.Y.Z - 适配 A.B.C.D`, wording unchanged), then empty the Unreleased section
- Commit flow: commit mod changes inside the CoeHarMod submodule (workspace branch) first, then update the submodule pointer in the main repo; the changelog updates land in the same main-repo commit as the mod change
- The mod depends on the main repo's modding capabilities (uniques/fields/constants); after merging upstream, check whether the mod is affected, bump the mod version and record it in the changelog
