...
- Style: one line per entry, short bullet points; when a version has several themes, group entries into emoji category subsections (e.g. `### ⚖️ Maintenance system rework`, `### 🔧 Dev tooling`; the ZH copy uses Chinese section titles like `### ⚖️ 快乐系统优化`, `### 🔧 开发工具`), follow the existing log
