...
- **Upstream changelog page**: the `English original (auto-synced)` section of `docs/zh/Community/Upstream-changelog.md` is embedded at build time from the repo-root `changelog.md` (see the `upstream-changelog` container in `docs-vitepress/.vitepress/config.ts`), so it stays current automatically after every upstream merge — never maintain an English copy by hand. The `Recent version translations` section is human-maintained and may lag; backfill it gradually, newest versions first.
