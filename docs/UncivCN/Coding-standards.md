...
- **30-word limit**: English counts words, the Chinese copy counts characters; links and punctuation excluded. Split big changes into multiple short entries - never one giant line.
...
- Style: one line per entry, short bullet points; when a version has several themes, group entries into emoji category subsections (e.g. `### ⚖️ Maintenance system rework`, `### 🔧 Dev tooling`; the ZH copy uses Chinese section titles like `### ⚖️ 快乐系统优化`, `### 🔧 开发工具`), follow the existing log. Entries are player-facing too - describe what mod users see, not implementation details - and follow the same 30-word limit; split big changes into multiple entries.
